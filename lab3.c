    ...
    while (1) {
        ...
        if (n > 0) {
            ...
            if (strncmp(buf, "GET / ", 6) == 0 || strncmp(buf, "GET /HTTP", 9) == 0) {
                // root URL → just send the control page
                send(client_fd, html_page, sizeof(html_page) - 1, 0);
            }
            else if (sscanf(buf, "GET /%d", &on) == 1) {
                gpio_put(LED_PIN, on);
                send(client_fd, html_page, sizeof(html_page) - 1, 0);
            }
else {
...
{
    while(1) {
        printf("Timer period = %d us. New value?", timer_delay) ;
        fscanf(stdin,"%d",&timer_delay) ;
    }
}
